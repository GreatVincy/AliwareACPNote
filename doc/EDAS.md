...
`
上述命令默认安装JDK1.8, 如果要使用JDK1.7, 需要加上`-force -java7`参数
2. 方式二:   
...
## 白皮书重点收集
+ 企业级分布式应用服务 EDAS 由控制台、数据采集系统、配置注册中心和鉴权中心等
系统组成
+ EDAS 控制台包含 2 个组件，EDASConsole(界面) 和 EDASAdmin(后台任务:定时同步 ECS 数据、定时自动扩
缩容等)
+ 数据收集系统包含:
